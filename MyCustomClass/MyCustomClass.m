...
{
    [SVProgressHUD showWithStatus:stringMessage];
...
{
    //[SVProgressHUD dismissWithDelay:timeDalay];
    [SVProgressHUD showErrorWithStatus:stringMessage];
}
...
+(void)logApi:(NSString *)pageStr
{

//    NSLog(@"111%@",[historyModel sharedhistoryModel].userEmail);
//    NSLog(@"222%@",[historyModel sharedhistoryModel].uid);
//    NSLog(@"333%@",[historyModel sharedhistoryModel].deviceName);
//    NSLog(@"444%@",[historyModel sharedhistoryModel].deviceID);
//    NSLog(@"555%@",[historyModel sharedhistoryModel].latitudeStr);
//    NSLog(@"666%@",[historyModel sharedhistoryModel].longitudeStr);
//    NSLog(@"777%@",[historyModel sharedhistoryModel].deviceOS);
    
    
    if (![historyModel sharedhistoryModel].userEmail)
    {
    [historyModel sharedhistoryModel].userEmail=@"";
    }
    if (![historyModel sharedhistoryModel].uid)
    {
        [historyModel sharedhistoryModel].uid=@"";
    } if (![historyModel sharedhistoryModel].deviceName)
    {
        [historyModel sharedhistoryModel].deviceName=@"";
    } if (![historyModel sharedhistoryModel].deviceID)
    {
        [historyModel sharedhistoryModel].deviceID=@"";
    } if (![historyModel sharedhistoryModel].latitudeStr)
    {
        [historyModel sharedhistoryModel].latitudeStr=@"";
    } if (![historyModel sharedhistoryModel].longitudeStr)
    {
        [historyModel sharedhistoryModel].longitudeStr=@"";
    } if (![historyModel sharedhistoryModel].deviceOS)
    {
        [historyModel sharedhistoryModel].deviceOS=@"";
    }
    
    NSDictionary*dataString=@{@"user_email":[historyModel sharedhistoryModel].userEmail,@"uid":[historyModel sharedhistoryModel].uid,@"action":@"log",@"pagetitle":pageStr,@"device_name":[historyModel sharedhistoryModel].deviceName,@"mobile":@"IOS ",@"op_greatplus_user_id":[historyModel sharedhistoryModel].opGreatplususerId,@"device_id":[historyModel sharedhistoryModel].deviceID,@"longitude":[historyModel sharedhistoryModel].latitudeStr,@"latitude":[historyModel sharedhistoryModel].longitudeStr,@"device_os":[historyModel sharedhistoryModel].deviceOS,@"uid":[historyModel sharedhistoryModel].uid,@"token":[historyModel sharedhistoryModel].token};
    NSLog(@"dataString>>>>>%@",dataString);

    
    if ([NSJSONSerialization isValidJSONObject:dataString]) {//validate it
        NSError* error;
        NSData* jsonData = [NSJSONSerialization dataWithJSONObject:dataString options:NSJSONWritingPrettyPrinted error: &error];
        NSString*urlstr=[NSString stringWithFormat:@"%@%@",[historyModel sharedhistoryModel].clientUrl,@"service/mobile-user-log.php"];
